...
async function gcpLogin(apiUrl, accessId) {
    ...
    let gcpCloudId;
    try {
        gcpCloudId = await akeylessCloud.getCloudId('gcp', gcpAudience);
    } catch (err) {
        console.error('akeylessCloud.getCloudId failed:', err);
        throw err;
    }
    console.log('we get the gcpCloudId: ', gcpCloudId);
